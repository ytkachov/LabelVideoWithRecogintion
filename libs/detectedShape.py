...
DEFAULT_FILL_COLOR = QColor(200, 20, 0, 100)
LINE_WIDTH = 2
TEXT_GAP = 3
MIN_Y_LABEL = 10
...
class DetectedShape(object):
    ...
    def __init__(self, label, score, extent):
        self.label = label
        self.score = score
        self.extent = extent
        self.selected = False
    ...
    def paint(self, painter):
        if self.extent:
            ...
            lbl = str.format('{0} - {1}%', self.label, str(int(self.score * 100.0)))
            ...
            rectf = QRectF(min_x - LINE_WIDTH, min_y - h - TEXT_GAP - LINE_WIDTH, w + TEXT_GAP, h + TEXT_GAP)
